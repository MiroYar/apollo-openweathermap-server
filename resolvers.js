const { fetchWeather, avg, toFarenheit, toCelsius, toKelvin } = require('./helper');
        ...
        getWeather: async (_, { cityName, countryCode }) => {
            ...
            return {
                ...
                fahrenheit_avg: toFarenheit(
                    avg(
                        list.map(weather => weather.main.temp),
                        list.length
                    )
                ),
                ...
                fahrenheit_max_avg: toFarenheit(
                    avg(
                        list.map(weather => weather.main.temp_max),
                        list.length
                    )
                ),
                ...
                temp_farenheit: list.map(weather => toFarenheit(weather.main.temp)),
